...
public:
    ...
    /// Gets the natural scale factor associated with the display
    virtual float GetScale() const;

    /// Calculates the required size of the window rectangle, based on the desired client size
    virtual void AdjustWindowSize(uint32_t& width, uint32_t& height) const;
    ...
    /// Occurs when the window's DPI scale changes
    typedef void ScaleChangedT(Display* display, float scale);
    Noesis::Delegate<ScaleChangedT>& ScaleChanged();
    ...
    /// Occurs when the app must save its state (like cached pipelines)
    typedef void SaveStateT(Display* display);
    Noesis::Delegate<SaveStateT>& SaveState();

protected:
    Noesis::Delegate<LocationChangedT> mLocationChanged;
    Noesis::Delegate<SizeChangedT> mSizeChanged;
    Noesis::Delegate<ScaleChangedT> mScaleChanged;
    ...
    Noesis::Delegate<SaveStateT> mSaveState;
